...
- **✨ 생성형 AI 배경 (선택)**: OpenAI **GPT Image 2**로 프롬프트 기반 배경 이미지 생성. 야구 카드에서는 AI가 야구장 분위기만 만들고 팀명·점수는 코드가 정확히 그림. `OPENAI_API_KEY` 미설정 시 자동으로 기본 배경으로 대체되므로 **AI 없이도 모든 기능이 동작함**
...
│  └─ generate-image.js      Vercel 서버리스 함수 (GPT Image 2 프록시)
